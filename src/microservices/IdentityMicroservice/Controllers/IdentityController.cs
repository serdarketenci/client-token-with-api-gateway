...
namespace IdentityMicroservice.Controllers
{
    ...
    public class IdentityController : ControllerBase
    {
        ...
        public ActionResult<string> Login([FromBody] User user, [FromQuery(Name = "d")] string destination = "frontend")
        {
            ...
            var token = _jwtBuilder.GetToken(u.Id, u.IsAdmin ? Role.Admin : Role.User);

            return Ok(token);
        }

        [HttpPost("register")]
        public ActionResult Register([FromBody] User user, [FromQuery(Name = "d")] string destination = "frontend")
        {
            var u = _userRepository.GetUser(user.Email);
